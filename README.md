...
Launch the program via "screen -R -D", to prevent the console being closed and the program stopping. ([crtl + a] and then [ctrl + d]) will detach the screen, the command "screen -R -D" will re-attach the console.
...
## Help
For any problems, issues and suggestions, contract work, etc, please contact us directly
...
If Encryption is enabled, the program will search for the piler encryption key in the following locations: /usr/local/etc/piler.key, /etc/mailpiler/piler.key, /etc/piler/piler.key, /etc/piler.key

You can specify the userconfig location directly with the option: "--key /key/file" or "-k /key/file"
...
### Jobs
This program was designed to run multiple concurrent jobs/processes/sub-processes. The default is 16. I do not recommend going lower than 16.
On an 8core CPU or a system with SSD's set this to 32 or higher, use the option: "--jobs 32" or "-j 32"
...
## Recovering mails with the attachments, using 32 jobs
```
./xshok-piler-archive-recovery.sh.x -o /datastore/export -j 32
```
...
* (-j) use 2 jobs
* (-o) output to the /datastore/test dir
* (-e) export the SQL query
* (-r) enable attachment de-duplication recovery
* (-n) rescue 10 mails
* (-v) be verbose
```
./xshok-piler-archive-recovery.sh.x -j 2 -o /datastore/test -e -r -n 10 -v
```
...
* (-j) use 32 jobs .. depends on your system
* (-o) output to the /datastore/export dir
* (-r) enable attachment de-duplication recovery
* (-c) continue processing
```
./xshok-piler-archive-recovery.sh.x -j 32 -o /datastore/export --recover -c
```
...
* (-j) use 32 jobs .. depends on your system
* (-o) output to the /datastore/export dir
* (-p) enable processing mode
```
/xshok-piler-archive-recovery.sh.x -j 32 -o /datastore/export --process
````
...
USAGE: xshok-piler-archive-recovery.sh.x [OPTIONS] -a /var/piler/store -o /datastore/export -w /var/xshok/work_dir -j 32 -r -c

-h, --help      Display this program's help and usage information

--version       Output programs version and date information

-j, --jobs XX   Optional: Maximum simultaneous Jobs to run, defaults to 16
...
-r, --recover   Optional: Enable Processing required to recover de-duplicated attachments

-e, --export    Export the SQL queries to files instead of running them on the MySQL server, only used with --recover option

-d, --dry       Optional: Dry run, will not actually do anything
...
-t, --temp /temp/path   Optional: Set a temporary directory, defaults to /dev/shm/xshok_temp or /tmp/xshok_tmp

-w, --work /work/path   Optional: Used for the continue and recover options: defaults to /output/path

-u, --usercfg /usercfg/file     Optional: Location of the Piler config, defaults to /etc/mailpiler/piler.conf, /etc/piler/piler.conf, /etc/piler.conf, /usr/local/etc/piler.conf

-k, --key /key/file     Optional: Location of the Piler encryption key, defaults to /usr/local/etc/piler.key, /etc/mailpiler/piler.key, /etc/piler/piler.key, /etc/piler.key
